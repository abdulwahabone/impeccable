...
- **The command rail** (`site/components/DocsSidebar.astro`, `docs-kinpaku.css`): on the docs pages the commands sit in the sidebar as the agent's own palette, a contained section drawn with one hairline on the paper, a filter line at its head, group captions inside, and the current page's command as the one raised paper cap, its gold dot lit. A gray track was tried and read as mud in a paper rail. Its text sits on the same 14px line as the lists above, whose headings are bold ink over muted links. Descended from `docs/concepts/docs-slash-menu.html`.
