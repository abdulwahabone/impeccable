...
- **The command rail** (`site/components/DocsSidebar.astro`, `docs-kinpaku.css`): on the docs pages the commands sit in the sidebar as the agent's own palette, a contained panel on the paper hardware (a recessed gray track, the paper strip's) with a filter line at its head, group captions inside, and the current page's command as a raised paper cap with the gold dot lit. Its text sits on the same 14px line as the lists above, whose headings are bold ink over muted links. Descended from `docs/concepts/docs-slash-menu.html`.
