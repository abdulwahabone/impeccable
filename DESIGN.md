...
- **The slash moment** (`site/components/SlashMoment.astro`, `slash-moment.css`, `site/scripts/components/slash-moment.js`): the docs index opens on the agent's composer, which types `/impeccable ` and unfolds the whole command set beneath the cursor. Arrow, type or click to browse; the visitor's page on one side and what the highlighted command makes of it on the other; return runs it. The composer and menu are the agent's chrome, so they sit on the instrument tokens; the rows are real links to each command's page. Spec in `docs/concepts/docs-slash-menu.html`.
